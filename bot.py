...
def _ensure_header(ws, header: list[str]) -> None:
    ...
    - 헤더가 완전히 다르면 강제 교체하지 않는다(사용자 커스텀 보호).
    - 다만, 기존 헤더가 원하는 헤더의 "앞부분(prefix)"과 동일하고 열이 부족한 경우에는
      ✅ 부족한 헤더 셀만 끝에 추가(확장)한다. (데이터 행은 건드리지 않음)
    """
    try:
        ...
        first_raw = list(values[0] or [])
        first = [str(c).strip() for c in first_raw]

        # 1) 완전 동일하면 OK
        if first == header:
            return

        # 2) 기존 헤더가 원하는 헤더의 prefix이면 → 부족한 헤더만 확장
        if first and len(first) < len(header) and first == header[: len(first)]:
            for col_1based in range(len(first) + 1, len(header) + 1):
                try:
                    ws.update_cell(1, col_1based, header[col_1based - 1])
                except Exception:
                    pass
            return

        # 3) 그 외(헤더 불일치)는 건드리지 않음
        return
    except Exception as e:
...
def append_site_export_rows(rows: list[list[str]]) -> bool:
    ...
    내부에서 항상 8컬럼([day,sport,src_id,title,body,createdAt,simple,comments])로 정규화한다.
    (site_export에는 comments를 기본적으로 비워둔다)
    """
    client_gs = get_gs_client()
    spreadsheet_id = os.getenv("SPREADSHEET_ID")
    if not (client_gs and spreadsheet_id):
...
def get_export_ws(sheet_name: str):
    ...
    try:
        ...
        try:
            desired_cols = max(int(getattr(ws, 'col_count', 0) or 0), 10, len(EXPORT_HEADER))
            if int(getattr(ws, 'col_count', 0) or 0) < desired_cols:
...
def append_export_rows(sheet_name: str, rows: list[list[str]]) -> bool:
    ...
    ✅ 컬럼 스펙
    - 입력 rows는 6컬럼([day,sport,src_id,title,body,createdAt]) 또는 7/8컬럼일 수 있음
    - 내부에서 항상 8컬럼([day,sport,src_id,title,body,createdAt,simple,comments])로 맞춘다.
      - simple: body에서 자동 생성
      - comments(H열): title 기반 OpenAI 댓글 5~6개(줄바꿈 구분)
    """
    if not rows:
        return True
    ...
    # 댓글 생성 옵션
    comment_enabled = (os.getenv("EXPORT_COMMENT_ENABLED", "1") or "1").strip().lower() not in ("0", "false", "no", "off")
    try:
        n_comments = int(os.getenv("EXPORT_COMMENT_COUNT", "6") or "6")
    except Exception:
        n_comments = 6
    ...
    for r in rows:
        ...
        if comment_enabled and not comments:
            t = str(title or "").strip()
            if not t and simple:
                t = str(simple).split("\n")[0].strip()
            comments = generate_export_comment_block(t, n=n_comments)
...
# ───────────────── OpenAI 댓글 생성 (export 탭 H열) ─────────────────

_EXPORT_COMMENT_CACHE: dict[str, str] = {}
_EXPORT_COMMENT_CACHE_MAX = int(os.getenv("EXPORT_COMMENT_CACHE_MAX", "500") or "500")


def _normalize_comment_lines(text: str, n: int) -> list[str]:
    """OpenAI 출력 텍스트를 '댓글 라인 리스트'로 정규화."""
    if not text:
        return []
...
def _fallback_comments_from_title(title: str, n: int = 6) -> str:
    """OpenAI 실패 시 최소 폴백(간단 변형)."""
    t = (title or "").strip()
    if not t:
        return ""
    # 너무 반복되지 않게 약간의 변형만 제공 (폴백은 rarely)
    bases = [
        f"{t} 분석 잘 봤습니다.",
        f"{t} 경기 기대되네요. 글 감사합니다.",
        f"{t} 내용 정리 깔끔해서 참고 잘 했어요.",
        f"{t} 흐름 설명이 이해하기 좋네요.",
        f"{t} 포인트 정리 감사합니다! 도움 됐습니다.",
        f"{t} 분석글 잘 읽고 갑니다.",
    ]
    return "\n".join(bases[: max(1, min(n, len(bases)))])


def generate_export_comment_block(title: str, n: int = 6) -> str:
    """export 탭 H열에 넣을 댓글 블록(줄바꿈 N개) 생성.

    - title 기반으로 한국어 댓글 n개를 생성해 반환(각 줄=댓글 1개)
    - OpenAI 사용 (실패 시 폴백)
    """
    title = (title or "").strip()
    if not title:
    ...
    n = max(3, min(10, int(n or 6)))

    cache_key = f"{title}||{n}"
    if cache_key in _EXPORT_COMMENT_CACHE:
        return _EXPORT_COMMENT_CACHE[cache_key]

    client_oa = get_openai_client()
    if not client_oa:
        out = _fallback_comments_from_title(title, n=n)
        _EXPORT_COMMENT_CACHE[cache_key] = out
        return out

    # title에서 리그/팀 구간(대략) 추출 → 프롬프트 힌트로만 사용
    league = ""
    try:
        m = re.search(r"\[(.*?)\]", title)
        if m:
            league = (m.group(1) or "").strip()
    except Exception:
        league = ""

    teams_hint = ""
    try:
        after = title.split("]", 1)[1] if "]" in title else title
        after = re.sub(r"^\s*\d{1,2}\s*월\s*\d{1,2}\s*일\s*", "", after).strip()
        after = re.sub(r"\s*(스포츠)?분석.*$", "", after).strip()
        teams_hint = after
    except Exception:
        teams_hint = ""
    ...
    prompt = f"""아래 경기 분석글 제목을 보고, 사람이 실제로 남긴 것처럼 자연스러운 한국어 댓글 {n}개를 작성해줘.

필수 조건:
- 각 줄은 댓글 1개 (총 {n}줄)
- 모든 댓글에 리그명과 두 팀명을 모두 포함 (제목에 있는 표기를 그대로 사용)
- 문장/어투/길이/표현을 서로 다르게 해서 반복 느낌이 없게
- 과한 홍보/해시태그/이모지/도박(토토/베팅) 언급 금지
- 번호 매기기/불릿/따옴표/머리말 없이 '댓글 문장만' 줄바꿈으로 출력

제목: {title}
리그 힌트: {league}
팀 힌트(두 팀 포함): {teams_hint}
"""
    ...
    try:
        resp = client_oa.chat.completions.create(
            model=model,
            messages=[
                {"role": "system", "content": "You write natural Korean community comments about sports match analysis posts."},
                {"role": "user", "content": prompt},
            ],
            temperature=float(os.getenv("EXPORT_COMMENT_TEMPERATURE", "0.9") or "0.9"),
        )
        text = (resp.choices[0].message.content or "").strip()
        lines = _normalize_comment_lines(text, n=n)

        # 부족하면 폴백으로 채우기(최소)
        if len(lines) < n:
            fb = _normalize_comment_lines(_fallback_comments_from_title(title, n=n), n=n)
            for s in fb:
                if len(lines) >= n:
                    break
                if s not in lines:
        ...
        print(f"[OPENAI][COMMENTS] 생성 실패 → 폴백 사용: {e}")
        out = _fallback_comments_from_title(title, n=n)

    # 캐시 크기 관리
    try:
        if len(_EXPORT_COMMENT_CACHE) >= _EXPORT_COMMENT_CACHE_MAX:
            _EXPORT_COMMENT_CACHE.clear()
    except Exception:
        pass
