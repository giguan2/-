...
_DEFAULT_SITE_FOOTER_VARIANTS = [
    f"이번 경기는 {_SITE_FOOTER_KW}을 참고해 더 정확한 정보를 확인하면 도움이 된다.",
    f"더 자세한 흐름과 변수는 {_SITE_FOOTER_KW}에서 추가로 확인해두면 좋다.",
    f"전력 비교와 관전 포인트는 {_SITE_FOOTER_KW}의 정리와 함께 보면 이해가 쉬워진다.",
    f"라인업·전술 변동까지 챙기려면 {_SITE_FOOTER_KW} 자료를 함께 참고하자.",
    f"최종 판단 전, {_SITE_FOOTER_KW}의 최신 정보도 한 번 더 점검해보면 좋다.",
    f"핵심 포인트를 빠르게 정리하려면 {_SITE_FOOTER_KW}의 추가 분석도 유용하다.",
    f"이번 매치업의 관전 포인트는 {_SITE_FOOTER_KW}를 함께 보면 더 선명해질 것이다.",
    f"부상·로테이션 같은 변수는 {_SITE_FOOTER_KW}에서 업데이트되는 내용을 참고하면 좋다.",
    f"경기 흐름을 더 정확히 읽기 위해 {_SITE_FOOTER_KW}의 분석을 함께 확인해보자.",
    f"승부를 가를 포인트는 {_SITE_FOOTER_KW}의 자료와 함께 보면 더 잘 보일 수 있다.",
    f"이번 경기는 {_SITE_FOOTER_KW}에서 제공하는 정보를 참고하면 판단에 도움이 된다.",
    f"세부 데이터와 맥락은 {_SITE_FOOTER_KW}에서 확인하면 더 명확해진다.",
    f"경기 전 체크리스트는 {_SITE_FOOTER_KW}의 정리와 함께 보면 더 꼼꼼해진다.",
    f"중요 변수 정리는 {_SITE_FOOTER_KW}의 최신 자료를 참고하면 도움이 될 것이다.",
    f"추가 정보가 필요하다면 {_SITE_FOOTER_KW}에서 관련 분석을 함께 참고하자.",
]
...
def rewrite_for_site_openai(
    ...
    *,
    ...
    if footer_line_env.lower() in ("0", "off", "false", "none", "no"):
        ...
        # ✅ 기본: \'스포츠분석 커뮤니티 오분\' 키워드를 포함한 문구를 매치업별로 자연스럽게 변형
