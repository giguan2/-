        ...
        async with httpx.AsyncClient(
            ...
            for page in range(start_page, end_page + 1):
                ...
                for item in items:
                    ...
                    # /crawlmazbaseball_tomorrow 실행 시 내일 경기가 아닌 글도 함께 저장될 수 있었다.
...
def _youtoo_find_header_index(old_header: list[str], col: str) -> int | None:
    ...
    위 경우 실제 데이터는 여전히 L=본문내용, M=본문길이, N=본문20자미만,
    O/P=수기 컬럼인 경우가 많아서, 위치 기반으로 우선 복원한다.
    """
    hdr = [str(c).strip() for c in list(old_header or [])]

    # 1) 위치 기반 특수 복원(헤더 꼬임 방지)
    try:
        if hdr[:11] == YOUTOO_AUTO_HEADER[:11]:
            # 과거 정상형: L=본문내용, M=본문20자미만, N/O=수기
            if len(hdr) >= 15 and hdr[11:15] == ["본문내용", "본문20자미만", "적중건제출여부", "지급여부"]:
                pos_map = {
                    "본문내용": 11,
                    "본문20자미만": 12,
                    "적중건제출여부": 13,
            ...
            # 정상형: L=본문내용, M=본문길이, N=본문20자미만, O/P=수기
            if len(hdr) >= 16 and hdr[11:16] == ["본문내용", "본문길이", "본문20자미만", "적중건제출여부", "지급여부"]:
                pos_map = {
                    "본문내용": 11,
                    "본문길이": 12,
                    "본문20자미만": 13,
            ...
            # 꼬인 헤더: L/M이 manual로 덮였지만 데이터는 L/M/N/O/P에 살아있는 경우
            if len(hdr) >= 16 and hdr[11:13] == YOUTOO_MANUAL_HEADER and "본문20자미만" in hdr[11:16]:
                last_submit = max((i for i, v in enumerate(hdr[:16]) if v == "적중건제출여부"), default=None)
                last_pay = max((i for i, v in enumerate(hdr[:16]) if v == "지급여부"), default=None)
                short_pos = next((i for i, v in enumerate(hdr[11:16], start=11) if v == "본문20자미만"), None)
                pos_map = {
                    "본문내용": 11,
                    "본문길이": 12,
                    "본문20자미만": short_pos,
                ...
                if col in pos_map and pos_map[col] is not None:
                    return pos_map[col]
    except Exception:
        pass
...
def _quiz_alias_pairs() -> dict[str, str]:
    ...
    default = {
        ...
        "SSG": ["SSG", "ssg", "쓱", "SSG랜더스", "쓱랜더스", "랜더스", "스크"],
